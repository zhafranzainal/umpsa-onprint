...
<div class="flex flex-wrap">
    ...
    <x-inputs.group class="w-full">
        <x-inputs.select name="package_id" label="Package" required>
            @php $selected = old('package_id', ($editing ? $order->package_id : '')) @endphp
    ...
    <x-inputs.group class="w-full">
        <x-inputs.text
            name="document_file"
            label="Document File"
            :value="old('document_file', ($editing ? $order->document_file : ''))"
            maxlength="255"
    ...
    <x-inputs.group class="w-full">
        <x-inputs.number
            name="point"
            label="Point"
            :value="old('point', ($editing ? $order->point : ''))"
            max="255"
            placeholder="Point"
            required
        ></x-inputs.number>
    ...
    <x-inputs.group class="w-full">
        <x-inputs.textarea
            ...
            maxlength="255"
            required
            >{{ old('qr_code', ($editing ? $order->qr_code : ''))
            }}</x-inputs.textarea
        >
    </x-inputs.group>
</div>
